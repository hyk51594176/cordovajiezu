<template>
    <div>
        <div id="map"
            ...
            <div></div>
            <div class="swpier"
                 v-show='swipershow&&busiInfoList.length>0'>
                <transition name="vux-popup-spwier">
                    <swiper height="130px"
                        ...
                        <swiper-item class="swiper-img"
                                     v-for="(marker,i) in busiInfoList"
                                     :key='marker.busiInfoId'>
                            <div class="itme-div">
                                <div class="itme-div-title">
                                    <span :style="{color:$store.state.themestyle.theme.themeColor}">{{marker.binfoName}}</span>
        ...
        <nav class="mtabar"
             :style='theme.tabbar'>
            <div @click=goto(busiInfoList[index])
                 tag='div'
                 class="detail">
...
export default {
    ...
    watch: {
        ...
        index(v) {
            if (map && this.busiInfoList.length > 0) {
                let obj = this.busiInfoList[v]
                this.setMarker(obj)
            }
        ...
        marker(m) {
            if (m && map) {
                map.animateCamera({
                    'target': {
                        lat: m.lat || m.binfoLatitude,
                        lng: m.lng || m.binfoLongitude
                    },
                    'tilt': 45,
                    'zoom': 13,
                    'bearing': 0,
        ...
        busiInfoList(arr) {
            ...
            arr.forEach((obj, index) => {
                if (obj.busiInfoId == this.marker.busiInfoId) {
                    this.index = index
                }
            });
    ...
    methods: {
        ...
        regionalBusiness(camera) {
            if (camera.zoom > 9 && this.cameraHashCode != camera.hashCode) {
                this.getVisibleRegion()
                this.cameraHashCode = camera.hashCode
            }
        },
        getVisibleRegion() {
            //获取地图东南，西北的经纬度
        ...
        clearBusInfoMarkers() {
            this.busiInfoMarkers.forEach((marker, index) => {
                let flag = this.currentList.some(obj => {
        ...
        //marker 点击事件
        markerClick(marker) {
            this.setIndex(marker)
        },
        //marker弹窗点击事件
        infoClick(marker, index) {
            this.setIndex(marker)
            this.swipershow = true
        },
        setIndex(marker) {
            this.busiInfoList.forEach((obj, index) => {
                if (obj.busiInfoId == marker.busiInfoId) {
                    this.index = index
                }
            });
        },
        ...
        prv() {
            if (this.index === 0) {
                this.index = this.busiInfoList.length - 1
            } else {
                this.index -= 1
            }
        },
        //下一个滑动窗口
        next() {
            if (this.index === this.busiInfoList.length - 1) {
                this.index = 0
            } else {
                this.index += 1
            }
        ...
        missionDetail(){
            console.log(this.marker.areaId)
            this.missionList.forEach( mission=> {
